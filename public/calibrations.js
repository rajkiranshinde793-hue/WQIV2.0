...
// Global Cancel Function
    function cancelCalibration() {
        ...
        // --- THE FIX: Send an explicit cancel command instead of "none" ---
        set(triggerRef, "cancel_cal"); 
    ...
    btnPh.addEventListener("click", () => {
        ...
        set(triggerRef, "lock_ph"); // <--- ADD THIS LINE TO LOCK POWER
        document.querySelectorAll('.calib-btn').forEach(b => b.classList.remove('active-btn'));
    ...
    btnTds.addEventListener("click", () => {
        ...
        set(triggerRef, "lock_tds"); // <--- ADD THIS LINE TO LOCK POWER
    ...
    function renderTurbPoint() {
        ...
        document.getElementById("turb-measure-btn").addEventListener("click", () => {
            ...
            set(ref(database, 'settings/calibration/target_value'), val); 
            // THIS IS THE CRITICAL FIX: Change "turb_zero" to "turb_cal"
            executeCalibrationStep("turb_cal", `Reading & Calibrating Sensor to ${val} NTU`, () => {
