    ...
    function generateProfessionalPDF(dataArray, mode) {
        ...
        doc.setFontSize(18);
        doc.text("JALNETRA WATER TEST REPORT", 105, 18, { align: "center" });
     ...
     { name: "EC", value: data.ec, unit: "µS/cm", limit: "< 1500" },

     { name: "TSS", value: data.tss, unit: "mg/L", limit: "< 50" },

     { name: "Salinity", value: data.salinity, unit: "ppt", limit: "< 0.5" }
];

            parameters.forEach(p => {
                let status = "PASS";

             if (p.name === "pH" && p.value !== undefined && (p.value < 6.5 || p.value > 8.5))
             status = "FAIL";

             if (p.name === "TDS" && p.value !== undefined && p.value > 500)
             status = "FAIL";

             if (p.name === "Turbidity" && p.value !== undefined && p.value > 5)
             status = "FAIL";

             if (p.name === "Hardness" && p.value !== undefined && p.value > 250)
             status = "WARNING";

             if (p.name === "TSS" && p.value !== undefined && p.value > 50)
             status = "WARNING";

             if (p.name === "Salinity" && p.value !== undefined && p.value > 0.5)
             status = "WARNING";

/* EC should not show PASS/FAIL */
if (p.name === "EC") status = "-";
             ...
             else if (status === "PASS") doc.setTextColor(0,150,0);
